package com.tho.serialization.buffer;
...
import com.tho.thojlib.bit.BitUtils;
...
public class BitBuffer {
	...
	public void putBoolean(final boolean value) {
		put(value ? BitBuffer.BOOLEAN_TRUE : BitBuffer.BOOLEAN_FALSE, BitBuffer.BOOLEAN_BIT_SIZE);
	}

	public boolean getBoolean() {
		return get(BitBuffer.BOOLEAN_BIT_SIZE) == BitBuffer.BOOLEAN_TRUE;
	}
	...
	public long get2(final int size) {
		// Check arguments
		if (size > Long.SIZE) {
			throw new IllegalArgumentException("Can't get more than " + Long.SIZE + "bits (arg is " + size + "bits)");
		}
		if (this.bitPosition - size < 0) {
			throw new BufferOverflowException();
		}
		...
		while (remainingBitToRead > 0) {
			...
			// Compute number of bit(s) to read in this part of the buffer
			if (remainingBitToRead > MathUtils.modPowerOfTwo(this.bitPosition - 1, BitUtils.BITS_PER_BYTE)) {
				nbOfBitToRead = MathUtils.modPowerOfTwo(this.bitPosition - 1, BitUtils.BITS_PER_BYTE) + 1;
			} else {
				nbOfBitToRead = remainingBitToRead;
			}
			...
			result |= (this.byteBuffer[this.bitPosition >> 3] & BitUtils.generateBitmask(
					MathUtils.modPowerOfTwo(this.bitPosition, BitUtils.BITS_PER_BYTE), nbOfBitToRead)) >> MathUtils
							.modPowerOfTwo(this.bitPosition, BitUtils.BITS_PER_BYTE) << remainingBitToRead
									- nbOfBitToRead;
